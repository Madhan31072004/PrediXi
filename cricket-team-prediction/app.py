...
try:
    ...
    firebase_admin.initialize_app(cred, {
        'databaseURL': 'https://predixi-78708-default-rtdb.firebaseio.com/'
    })
...
def preprocess_data(players_df):
    try:
        ...
        # Select numeric feature columns, excluding 'Points'
        feature_cols = players_df.select_dtypes(include=[np.number]).columns.drop('Points')
        if feature_cols.empty:
            app.logger.warning("No numeric feature columns found, using default points")
            return None, players_df.index, None

        # Prepare the feature matrix X and target vector y
        X = players_df[feature_cols].fillna(0)  # Fill NaN values with 0 for features
        y = players_df['Points'].fillna(0) if 'Points' in players_df.columns else None  # Fill NaN values with 0 for Points

        if y is None or y.isna().all():
            app.logger.warning("No valid points column, using default points")
            return None, players_df.index, None

        # Return the original feature matrix and target vector without scaling
        return X, y, None  # Return None for scaler since we are not scaling
    except Exception as e:
...
def predict_teams(selected_players, original_players_df):
    try:
        ...
        if 'points' in original_players_df.columns and not original_players_df['points'].isna().all():
            ...
            # Map existing points and roles from original dataset
            ...
            app.logger.debug("Points missing, predicting with ML or using defaults")

            # Check if ML models can be used
            X, y = preprocess_data(original_players_df)  # No scaling
            if X is not None and y is not None:
                dt_model = train_decision_tree(X, y)
                xgb_model = train_xgboost(X, y)

                if dt_model and xgb_model:
                    feature_cols = original_players_df.select_dtypes(include=[np.number]).columns.drop('points', errors='ignore')
                    selected_X = X.loc[selected_players['PlayerName'].map(original_players_df.set_index('playername')['playername'])]
                    dt_preds = dt_model.predict(selected_X)
                    xgb_preds = xgb_model.predict(selected_X)

                    # Average predictions and assign them directly
                    selected_players['Points'] = (dt_preds + xgb_preds) / 2
                else:
                    app.logger.warning("ML models failed, using default points")
                    selected_players['Points'] = np.linspace(7.0, 9.0, len(selected_players))
            else:
                app.logger.warning("No valid data for ML, using default points")
                selected_players['Points'] = np.linspace(7.0, 9.0, len(selected_players))

            # Assign roles as 'Unknown' if not found
        ...
        if 'photo' in original_players_df.columns:
            ...
            # Ensure proper filename format (lowercase, underscores)
            original_players_df['photo'] = original_players_df['photo'].str.lower().str.replace(' ', '_')

            # Prepend 'images/' to match static folder structure
            original_players_df['photo'] = 'images/' + original_players_df['photo']

            # Create a dictionary mapping player names to their photo paths
            photo_mapping = original_players_df.set_index('playername')['photo'].to_dict()

            # Apply mapping to selected players
            selected_players['Photo'] = selected_players['PlayerName'].map(photo_mapping).fillna('images/default.jpg')

            # Debugging final paths
            app.logger.debug(f"Final Assigned Photos: {selected_players[['PlayerName', 'Photo']].to_dict(orient='records')}")
